...
// - Iterate only on cells that may become alive in the current time-step (only the ones in range 2 from alive cells)
// - Thread-local buckets minimize synchronization and improve parallelism
// - Guided scheduling adapts to workload imbalance from sparse regions
...
void simulate_parallel4(Grid_better &g){
  ...
  // try different sizes !!
  const int RES_B = tot_cells / THREADS;  
  ...
  // gather the local buckets into a flat vector 

  // obtain the offsets via sequential prefix sum
  int total = 0;
  ...
  // __gnu_parallel::sort(flat_vec_dead.begin(), flat_vec_dead.end()); 
  // std::sort(flat_vec_dead.begin(), flat_vec_dead.end());
  std::sort(std::execution::par, flat_vec_dead.begin(), flat_vec_dead.end());
  ...
  for (long step = 0; step < MAX_STEPS; ++step){
    ...
    // parallelize, work is separated among threads for all the time-step, 
    // at the end reduce to obtain changes
    #pragma omp parallel reduction(+ : changes)
    {
      ...
      // iterate on cells that may become alive
